...
#[derive(Copy, Clone, Ord, PartialOrd, Eq, PartialEq, Hash, Debug)]
pub enum EnviByteOrder {
    Intel = 0,
    Network = 1,
}

impl FromStr for EnviByteOrder {
    type Err = ();
...
impl Display for ParseEnviHeaderError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ParseEnviHeaderError::BadValue(e) => {
                writeln!(f, "Failed to parse field '{}'.", e)
            }
            ParseEnviHeaderError::NoKey(line) => {
                writeln!(f, "Key not found for line {}!", line)
            }
            ParseEnviHeaderError::NoValue(line) => {
                writeln!(f, "Value not found for line {}!", line)
            }
            ParseEnviHeaderError::DuplicateKey(k) => {
                writeln!(f, "Duplicate key {}!", k)
            }
            ParseEnviHeaderError::RequiredFieldNotFound(k) => {
                writeln!(f, "Required field not found {}!", k)
            }
            ParseEnviHeaderError::InvalidFirstLine => {
...
impl Error for ParseEnviHeaderError {}

impl FromStr for EnviHeaders {
    type Err = ParseEnviHeaderError;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        for (number, text) in s.lines().enumerate() {
            if number == 0 {
                if text != "ENVI" {
                    return Err(ParseEnviHeaderError::InvalidFirstLine)
                }
            } else {
                ...
                let key = split.next()
                    .map(|x| Ok(x))
                    .unwrap_or(Err(ParseEnviHeaderError::NoKey(number)))?
                    .trim();

                let value = split.next()
                    .map(|x| Ok(x))
                    .unwrap_or(Err(ParseEnviHeaderError::NoValue(number)))?
                    .trim();
...
fn parse_scalar_field<T>(
    ...
    field: &'static str,
) -> Result<T, ParseEnviHeaderError>
    where T: FromStr
{
    fields_map.remove(field)
        .map(|x| Ok(x))
        .unwrap_or(Err(ParseEnviHeaderError::RequiredFieldNotFound(field)))?
        .parse::<T>()
        .map_err(|_| ParseEnviHeaderError::BadValue("bands"))
}