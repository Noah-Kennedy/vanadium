...
pub trait BatchedPixelReduce<T> {
    fn fold_over_batched_pixels<F, A>(&mut self, name: &str, accumulator: A, f: F) -> GenericResult<A>
          ...
          + 'static
{
    fn means(&mut self) -> GenericResult<Array1<T>> {
        ...
        let mut res = self.fold_over_batched_pixels("mean", accumulator, |pixels, acc| {
    ...
    fn std_deviations(&mut self, means: &Array1<T>) -> GenericResult<Array1<T>> {
        ...
        let mut res = self.fold_over_batched_pixels("std", accumulator, |pixels, acc| {
    ...
    fn covariance_matrix(&mut self, means: Option<&Array1<T>>, std_devs: Option<&Array1<T>>) -> GenericResult<Array2<T>> {
        ...
        let mut res = self.fold_over_batched_pixels("cov", accumulator, |pixels, acc| {
