...
PCA='hyperspectra-cli pca -i /data/bench-data/small.{type} -h /data/bench-data/small.{type}.hdr -o /data/small.pca.{type} -d 3 --min 0.0 --max 1.0'
...
time hyperfine --warmup=2 \
  --prepare 'rm *.png *.bil *.bip *.bsq || true' \
  --export-markdown BENCHMARKS_PCA_WARM.md \
  -L type bip,bsq \
  "$PCA"
...
time hyperfine --warmup=2 \
  --prepare 'rm *.png *.bil *.bip *.bsq || true; sync; echo 3 | tee /proc/sys/vm/drop_caches' \
  --export-markdown BENCHMARKS_PCA_COLD.md \
  -L type bip,bsq \
  "$PCA"

rm /data/*.png /data/*.bil /data/*.bip /data/*.bsq || true
